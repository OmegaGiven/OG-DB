...
const COLUMNS=[
 ...
 {id:'vol',label:'Build volume',sort:d=>d.buildVolumeL??1e9,cell:d=>`<span class="price">${esc(d.buildVolumeMm)}</span><span class="sub">${d.buildVolumeL!=null?(d.buildVolumeL+' L'+(d.priceUSD?` &middot; $${Math.round(d.priceUSD/d.buildVolumeL)}/L`:'')):(/belt|∞|infinite/i.test(d.buildVolumeMm||'')?'infinite length &mdash; belt':'')}</span>`},
